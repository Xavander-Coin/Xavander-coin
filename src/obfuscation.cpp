...
/* *** BEGIN OBFUSCATION MAGIC - XAVANDER **********
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Xavander and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 XAVANDER per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeXavanderAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100XAVANDER+1 ( bit on if present )
    // bit 1 - 10XAVANDER+1
    // bit 2 - 1XAVANDER+1
    // bit 3 - .1XAVANDER+1
    // bit 3 - non-denom
...
bool CObfuScationSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey)
{
    ...
    if (GetTransaction(vin.prevout.hash, txVin, hash, true)) {
        BOOST_FOREACH (CTxOut out, txVin.vout) {
			if (out.nValue == Params().MasternodeCollateralAmt() * COIN) { // Was 20000
            //if (out.nValue == 10000 * COIN) { // Was 20000
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("xavander-obfuscation");
