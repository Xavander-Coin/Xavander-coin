...
Value getblock(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "  \"merkleroot\" : \"smrtcx\", (string) The merkle root\n"
...
Value getblockchaininfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"chain\": \"smrtcx\",        (string) current network name as defined in BIP70 (main, test, regtest)\n"
            "  \"blocks\": smrtcsmrtc,         (numeric) the current number of blocks processed in the server\n"
            "  \"headers\": smrtcsmrtc,        (numeric) the current number of headers we have validated\n"
            "  \"bestblockhash\": \"...\", (string) the hash of the currently best block\n"
            "  \"difficulty\": smrtcsmrtc,     (numeric) the current difficulty\n"
            "  \"verificationprogress\": smrtcx, (numeric) estimate of verification progress [0..1]\n"
            "  \"chainwork\": \"smrtcx\"     (string) total amount of work in active chain, in hexadecimal\n"
            "}\n"
            "\nExamples:\n" +
...
Value getchaintips(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"height\": smrtcx,         (numeric) height of the chain tip\n"
            "    \"hash\": \"smrtcx\",         (string) block hash of the tip\n"
            ...
            "    \"status\": \"smrtcx\"        (string) status of the chain (active, valid-fork, valid-headers, headers-only, invalid)\n"
            "  }\n"
            "]\n"
...
Value getmempoolinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"size\": smrtcxx                (numeric) Current tx count\n"
            "  \"bytes\": smrtcxx               (numeric) Sum of all tx sizes\n"
            "}\n"
            "\nExamples:\n" +
